        ...
        if isinstance(self.object_to_serialize, dict):
            for attr_name, field in self.declared_fields.items():
                if field.name is not None:
                    raw_value_to_serialize = self.object_to_serialize.get(field.name, False)
                else:
                    raw_value_to_serialize = self.object_to_serialize.get(attr_name, False)
                if raw_value_to_serialize:
                    dict_response[attr_name] = field.get(raw_value_to_serialize)
...
if __name__ == "__main__":
    ...
    fake_mongo_query_result = {
        "_id":1234,
        "name":"Ur Mom"
    }

    obj = UserSerializer(fake_mongo_query_result)
    print(obj.data)